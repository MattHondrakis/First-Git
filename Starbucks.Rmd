...
temp_star <- starbucks %>%
  filter(size == "venti") %>%
  mutate(temp = as.factor(ifelse(grepl('Iced|Frappuccino', product_name), "Cold", "Hot")))
...
temp_star %>%
  filter(temp == "Hot" & serv_size_m_l > 650) %>%
  count(product_name, sort = TRUE)

temp_star %>%
  group_by(temp, serv_size_m_l) %>%
  summarize(n = n()) %>%
  ggplot(aes(temp, n, fill = as.factor(serv_size_m_l))) + geom_col(position = "dodge")
```
