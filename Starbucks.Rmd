...
# Carbs by fiber and sugar: CONTINUE LATER
...
```{r}
starbucks %>%
  filter(size == "venti" & grepl('Iced|Frappuccino', product_name)) %>%
  group_by(serv_size_m_l) %>%
  count()

starbucks %>%
  filter(size == "venti" & serv_size_m_l == 591) %>%
  count(product_name, sort = TRUE)

starbucks %>%
  select(product_name, sugar_g, total_carbs_g) %>%
  filter(15 < sugar_g & sugar_g <25 & total_carbs_g < 15) %>%
  arrange(-total_carbs_g)
